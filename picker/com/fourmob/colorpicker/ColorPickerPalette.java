...
public class ColorPickerPalette extends TableLayout {
	...
	public ContourPickerSwatch.OnContourSelectedListener mOnContourSelectedListener;
	...
	private ContourPickerSwatch createContourSwatch(int contour, int selected) {
		ContourPickerSwatch contourPickerSwatch = new ContourPickerSwatch(getContext(), contour, contour == selected, this.mOnContourSelectedListener);
		TableRow.LayoutParams layoutParams = new TableRow.LayoutParams(this.mSwatchLength, this.mSwatchLength);
		layoutParams.setMargins(this.mMarginSize, this.mMarginSize, this.mMarginSize, this.mMarginSize);
		contourPickerSwatch.setLayoutParams(layoutParams);
		return contourPickerSwatch;
	...
	public void drawPaletteByContour(int[] contours, int selectedColor) {
		if (contours == null) {
			return;
		}
		...
		for (int contour : contours) {
			...
			View colorSwatch = createContourSwatch(contour, selectedColor);
			setSwatchDescription(rowNumber, tableElements, rowElements, contour == selectedColor, colorSwatch);
	...
	public void initByContour(int size, int numColumns, ContourPickerSwatch.OnContourSelectedListener onContourSelectedListener) {
		this.mNumColumns = numColumns;
		Resources resources = getResources();
		if (size == 1) {
			this.mSwatchLength = resources.getDimensionPixelSize(R.dimen.color_swatch_large);
